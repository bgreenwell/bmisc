...
battery_target<- function(X, ...) {
  ...
  rownames(vi_matrix) <- colnames(vi_matrix) <- names(X)
  ...
  for (i in seq_len(nvars)) {
    ...
    if (is.null(trees[[i]])) {
      ...
      vi_matrix[i, -i] <- if (is.null(trees[[i]]$splits)) 0 else vi
  ...
  out <- list("results" = data.frame("Target" = names(X),
                         ...
                         "N.vars" = num_vars),
              "vi_matrix" = round(vi_matrix, digits = 0),
              "trees" = trees)
